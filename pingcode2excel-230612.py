...
def devResult(developerDict, scoresDict, df, filename):
    ...
    with pd.ExcelWriter(filename, engine='openpyxl', mode='a') as writer:
       developerdf.to_excel(writer, sheet_name='developer', index = False, encoding = 'utf-8')

def testResult(testerDict, scoresDict, df, filename):
    tmpScoreList = []
    df = df.drop (df [df ['解决方案'] == '重复' ].index)
    ...
    with pd.ExcelWriter(filename, engine='openpyxl', mode='a') as writer:
       testerdf.to_excel(writer, sheet_name='tester', index = False, encoding = 'utf-8')
...
def main():
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='buglist', index = False, encoding = 'utf-8')
